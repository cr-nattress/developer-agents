...
# Configure logging - use a file handler only to avoid mixing with print output
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(name)s - %(levelname)s - %(message)s',
    handlers=[
        logging.FileHandler("orchestrator_clean_workflow.log")
    ]
)
...
def main():
    print("\n===== GIT ORCHESTRATOR CLEAN WORKFLOW EXAMPLE =====\n")
    ...
    try:
        ...
        branch_output = run_git_command(["checkout", "-b", branch_name], sandbox_path)
    ...
    try:
        stage_output = run_git_command(["add", "."], sandbox_path)
    ...
    try:
        commit_output = run_git_command(["commit", "-m", commit_message], sandbox_path)
    ...
    try:
        push_output = run_git_command(["push", "origin", branch_name], sandbox_path)
    ...
    print("\n===== GIT ORCHESTRATOR CLEAN WORKFLOW EXAMPLE COMPLETE =====\n")
